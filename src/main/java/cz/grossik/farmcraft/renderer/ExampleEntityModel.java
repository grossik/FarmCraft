...
@OnlyIn(Dist.CLIENT)
public class ExampleEntityModel extends SegmentedModel<ExampleEntity> {
    ...
    */
   public void setRotationAngles(ExampleEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      if (entityIn.getEating()) {
          this.batHead.rotateAngleX = headPitch * ((float)Math.PI / 180F);
          this.batHead.rotateAngleY = (float)Math.PI - netHeadYaw * ((float)Math.PI / 225F);
      ...
      } else if(entityIn.getIsBatHanging()) {
          this.batHead.rotateAngleX = headPitch * ((float)Math.PI / 180F);
